...
// Generated by `make zon` via `tools/gen_zon_files/`.
const root_manifest_rel_path = ".build/gen_zon_files/root_apps_manifest.json";
const esp_manifest_rel_path = ".build/gen_zon_files/esp_apps_manifest.json";
...
pub fn build(b: *std.Build) void {
    ...
    var manifest = apps_manifest.loadFromAbsolutePath(b.allocator, root_manifest_path) catch |err| switch (err) {
        error.FileNotFound => std.debug.panic(
            "missing generated apps manifest '{s}'; run `make zon` first",
            .{root_manifest_path},
        ),
        else => std.debug.panic("load generated apps manifest '{s}': {}", .{ root_manifest_path, err }),
    };
    ...
    if (countAppDependencies(b) != manifest_apps.len) {
        std.debug.panic(
            "generated apps manifest '{s}' does not match root build.zig.zon; run `make zon`",
            .{root_manifest_path},
        );
    }

    const root = b.build_root.handle;
    const test_step = b.step("test", "Run zig build test in each package listed in the generated apps manifest");
    for (manifest_apps) |app| {
        const build_file_rel = b.fmt("{s}/build.zig", .{app.root_path});
        root.access(build_file_rel, .{}) catch {
            std.debug.panic(
                "generated apps manifest entry '{s}' points to missing package '{s}'; run `make zon`",
