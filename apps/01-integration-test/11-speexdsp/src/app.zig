...
pub fn run(comptime ctx: type, comptime grt: type) !void {
    ...
    log.info("starting speexdsp integration runner", .{});

    var runner = testing.T.new(grt.std, grt.time, .compat_tests);
    defer runner.deinit();

    runner.timeout(480 * glib.time.duration.Second);
    runner.run("speexdsp/integration", speexdsp.test_runner.integration.make(grt));

    const passed = runner.wait();
    log.info("speexdsp integration runner finished", .{});
