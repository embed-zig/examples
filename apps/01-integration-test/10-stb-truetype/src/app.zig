...
pub fn run(comptime ctx: type, comptime grt: type) !void {
    ...
    log.info("starting stb_truetype integration runner", .{});

    var runner = testing.T.new(grt.std, grt.time, .compat_tests);
    defer runner.deinit();

    runner.timeout(480 * glib.time.duration.Second);
    runner.run("stb_truetype/integration", stb.test_runner.integration.make(grt));

    const passed = runner.wait();
    log.info("stb_truetype integration runner finished", .{});
