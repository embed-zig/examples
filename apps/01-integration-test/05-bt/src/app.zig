...
pub fn run(comptime runtime: type) !void {
    ...
    app_log.info("starting bt integration runner", .{});

    var runner = testing.T.new(std, .compat_tests);
    defer runner.deinit();

    runner.timeout(480 * std.time.ns_per_s);
    runner.run("bt/integration", bt.test_runner.integration.make(std, runtime.Channel));

    const passed = runner.wait();
    app_log.info("bt integration runner finished", .{});
