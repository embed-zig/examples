...
const ledstrip = @import("embed").ledstrip;
...
pub fn run(comptime ctx: type, comptime grt: type) !void {
    ...
    log.info("starting ledstrip animator unit runner", .{});

    var runner = testing.T.new(grt.std, grt.time, .compat_tests);
    defer runner.deinit();

    runner.timeout(240 * glib.time.duration.Second);
    runner.run("ledstrip/animator", ledstrip.test_runner.animator.make(grt));

    const passed = runner.wait();
    log.info("ledstrip animator unit runner finished", .{});
