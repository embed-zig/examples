...
pub fn run(comptime runtime: type) !void {
    ...
    app_log.info("starting ledstrip animator unit runner", .{});

    var runner = testing.T.new(std, .compat_tests);
    defer runner.deinit();

    runner.timeout(240 * std.time.ns_per_s);
    runner.run("ledstrip/animator", ledstrip.test_runner.animator.make(std));

    const passed = runner.wait();
    app_log.info("ledstrip animator unit runner finished", .{});
