const tests_embed = @import("tests_embed");
...
pub fn run(comptime runtime: type) !void {
    ...
    app_log.info("starting embed unit runner", .{});

    var runner = testing.T.new(std, .compat_tests);
    defer runner.deinit();

    runner.timeout(240 * std.time.ns_per_s);
    runner.run("embed/unit", tests_embed.make(std));

    const passed = runner.wait();
    app_log.info("embed unit runner finished", .{});
