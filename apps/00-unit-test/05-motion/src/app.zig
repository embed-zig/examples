...
const motion = @import("embed").motion;
...
pub fn run(comptime ctx: type, comptime grt: type) !void {
    ...
    log.info("starting motion unit runner", .{});

    var runner = testing.T.new(grt.std, grt.time, .compat_tests);
    defer runner.deinit();

    runner.timeout(240 * glib.time.duration.Second);
    runner.run("motion/unit", motion.test_runner.unit.make(grt));

    const passed = runner.wait();
    log.info("motion unit runner finished", .{});
